...
const schemaEntries = {
  ...
  ldapSyntaxes: helper.ReadCSVfile("./schema/ldapSyntaxes.csv", (row) =>
    row.join(",")
  ),
  matchingRules: helper.ReadCSVfile("./schema/matchingRules.csv", (row) =>
    row.join(",")
  ),
  matchingRuleUse: helper.ReadCSVfile("./schema/matchingRuleUse.csv", (row) =>
    row.join(",")
  ),
  attributeTypes: helper.ReadCSVfile("./schema/attributeTypes.csv", (row) =>
    row.join(",")
  ),
  objectClasses: helper.ReadCSVfile("./schema/objectClasses.csv", (row) =>
    row.join(",")
  ),
};
...
function guidToBytes(guid) {
  ...
  for (let i = 0; i < parts.length; i++) {
    const bytesInChar =
      i < 3 ? parts[i].match(/.{1,2}/g).reverse() : parts[i].match(/.{1,2}/g);
...
function renameEntryByUUID(db, UUID, newEntryDN) {
  let mergeBASEDN = Object.values(db).find(
    (g) => g.entryUUID === UUID && g.entryDN != newEntryDN
  );
  if (mergeBASEDN) {
...
function mergeDnBase(db) {
  renameEntryByUUID(
    db,
    "e927be8d-aab8-42f2-80c3-b2762415aed1",
    config.LDAP_BASEDN
  );
...
function mergeDnSambaDomainName(db) {
  ...
  var LDAP_SAMBA =
    "sambaDomainName=" + sambaDomainName + "," + config.LDAP_BASEDN;
...
function mergeDnUsers(db) {
  renameEntryByUUID(
    db,
    "3e01f47d-96a1-4cb4-803f-7dd17991c6bd",
    config.LDAP_USERSDN
  );
  ...
  db[config.LDAP_USERSDN] = {
    ...
    cn: config.LDAP_USERSDN.replace("," + config.LDAP_BASEDN, "").replace(
      "cn=",
      ""
    ),
...
function mergeDnDevices(db) {
  ...
  renameEntryByUUID(
    db,
    "ecc6dc2f-6816-4e50-bd6d-9e6e59593fab",
    config.LDAP_DEVICESDN
  );

  db[config.LDAP_DEVICESDN] = {
    // default values
    objectClass: "organizationalRole",
    cn: config.LDAP_DEVICESDN.replace("," + config.LDAP_BASEDN, "").replace(
      "cn=",
      ""
    ),
...
function mergeDnGroups(db) {
  renameEntryByUUID(
    db,
    "39af84ac-8e5a-483e-9621-e657385b07b5",
    config.LDAP_GROUPSDN
  );

  db[config.LDAP_GROUPSDN] = {
    // default values
    cn: config.LDAP_GROUPSDN.replace("," + config.LDAP_BASEDN, "").replace(
      "cn=",
      ""
    ),
...
function mergeDnUserDefaultGroup(db) {
  renameEntryByUUID(
    db,
    "938f7407-8e5a-48e9-a852-d862fa3bb1bc",
    config.LDAP_USERSGROUPSBASEDN
  );

  var usersGroupDn_hash =
    db[config.LDAP_USERSGROUPSBASEDN] &&
    db[config.LDAP_USERSGROUPSBASEDN].hasOwnProperty("gidNumber")
      ? db[config.LDAP_USERSGROUPSBASEDN].gidNumber.toString()
      : Math.abs(encode().value(config.LDAP_USERSGROUPSBASEDN)).toString();
  ...
  db[config.LDAP_USERSGROUPSBASEDN] = {
    ...
    cn: config.LDAP_USERSGROUPSBASEDN.replace(
      "," + config.LDAP_GROUPSDN,
      ""
    ).replace("cn=", ""),
    description: "Users default group",
    displayName: config.LDAP_USERSGROUPSBASEDN.replace(
      "," + config.LDAP_GROUPSDN,
      ""
    ).replace("cn=", ""),
  ...
  db[config.LDAP_USERSGROUPSBASEDN] = customizer.ModifyLDAPGroup(
    db[config.LDAP_USERSGROUPSBASEDN],
    {}
  );
}
...
function cleanUpOldEntries(db, compDate) {
  if (config.LDAP_DAYSTOKEEPDELETEDUSERS >= 0) {
    for (const [key, value] of Object.entries(db)) {
      if (
        value.hasOwnProperty("sAMAccountName") &&
        (!value.hasOwnProperty("modifyTimestamp") ||
          (value.hasOwnProperty("modifyTimestamp") &&
            helper.ldap_now_2_date(value.modifyTimestamp) < compDate))
      ) {
        helper.forceLog("database.js", "cleanUpOldEntries", "deleted user:", {
          entryDN: value.entryDN,
          modifyTimestamp: helper.ldap_now_2_date(value.modifyTimestamp),
          compDate: compDate,
        });
        delete db[key];
      } else if (
        value.hasOwnProperty("sambaGroupType") &&
        (!value.hasOwnProperty("modifyTimestamp") ||
          (value.hasOwnProperty("modifyTimestamp") &&
            helper.ldap_now_2_date(value.modifyTimestamp) < compDate))
      ) {
        helper.forceLog("database.js", "cleanUpOldEntries", "deleted group:", {
          entryDN: value.entryDN,
          modifyTimestamp: helper.ldap_now_2_date(value.modifyTimestamp),
          compDate: compDate,
        });
        ...
        value.hasOwnProperty("structuralObjectClass") &&
        value.hasOwnProperty("entryDN") &&
        value.entryDN.startsWith("ou=") &&
        (!value.hasOwnProperty("modifyTimestamp") ||
          (value.hasOwnProperty("modifyTimestamp") &&
            helper.ldap_now_2_date(value.modifyTimestamp) < compDate))
      ) {
...
async function refreshDBentries() {
  if (Date.now() > lastRefresh + refreshInterval) {
    ...
    let compDate = helper.ldap_now_2_date(
      helper.ldap_now(-1 * config.LDAP_DAYSTOKEEPDELETEDUSERS)
    );
...
async function mergeAzureGroupEntries(db) {
  ...
  for (let i = 0, len = groups.length; i < len; i++) {
    ...
    if (groupDisplayName !== groupDisplayNameClean) {
      helper.warn(
        "database.js",
        "group names may not contain any special chars. We are using ",
        groupDisplayNameClean,
        "instead of",
        groupDisplayName
      );
    }
    ...
    let group_hash =
      db[gpName] && db[gpName].hasOwnProperty("gidNumber")
        ? db[gpName].gidNumber.toString()
        : Math.abs(encode().value(group.id)).toString();
    ...
    helper.log(
      "database.js",
      "try fetching the members for group: ",
      group.displayName
    );
    ...
    if (members.length > 0) {
      ...
      helper.SaveJSONtoFile(
        members,
        "./.cache/members_" + groupDisplayNameClean + ".json"
      );
      helper.log(
        "database.js",
        "members_" + groupDisplayNameClean + ".json" + " saved."
      );
    }
  ...
  for (let i = 0, len = groups.length; i < len; i++) {
    ...
    if (db["tmp_nested_groups"][group.id]) {
      for (
        let j = 0, jlen = db["tmp_nested_groups"][group.id].length;
        j < jlen;
        j++
      ) {
        let g = db["tmp_nested_groups"][group.id][j];
        let gp = Object.values(db).find(
          (g) =>
            g.entryUUID === group.id && g.objectClass.includes("posixGroup")
        );
        let gpName = gp
...
async function mergeAzureUserEntries(db) {
  ...
  for (let i = 0, len = users.length; i < len; i++) {
    ...
    let isGuestOrExternalUser =
      user.userType == "Guest" ||
      user.identities.filter(
        (x) => x.hasOwnProperty("issuer") && x.issuer == "ExternalAzureAD"
      ).length > 0;
    let isExternalUserStateAccepted = user.externalUserState == "Accepted";
    let isMicrosoftAccount =
      isGuestOrExternalUser &&
      user.hasOwnProperty("identities") &&
      user.identities.filter(
        (x) => x.hasOwnProperty("issuer") && x.issuer == "ExternalAzureAD"
      ).length == 0;
    ...
    if (isGuestOrExternalUser && !isExternalUserStateAccepted) {
      helper.warn(
        "database.js",
        "mergeAzureUserEntries",
        "Guest user (#EXT#) has not yet accepted invitation",
        {
          mail: user.mail,
    ...
    // ignore personal microsoft accounts, because RPOC is not possible
      ...
      if (isGuestOrExternalUser && user.hasOwnProperty("mail")) {
        ...
        if (userPrincipalName.indexOf("#EXT#") > -1) {
          userPrincipalName = userPrincipalName.substring(
            0,
            userPrincipalName.indexOf("#EXT#")
          );
        } else {
          let issuers = user.identities.filter(
            (x) =>
              x.hasOwnProperty("issuer") && x.signInType == "userPrincipalName"
          );
          issuers.forEach(
            (issuer) =>
      ...
      if (userPrincipalName !== userPrincipalNameClean) {
        helper.warn(
          "database.js",
          "mergeAzureUserEntries",
          "userPrincipalNames may not contain any special chars. In a future version we are maybe using ",
          userPrincipalNameClean,
          "instead of",
          userPrincipalName
        );
      ...
      let userPrincipalNameOU = [userPrincipalName, ou]
        .filter((x) => typeof x === "string" && x.length > 0)
        .join(",");

      let upName = config.LDAP_USERRDN + "=" + user.mail + "," + udn;
      upName = upName.toLowerCase();

      renameEntryByUUID(db, user.id, upName);

      let user_hash =
        db[upName] && db[upName].hasOwnProperty("uidNumber")
          ? db[upName].uidNumber.toString()
          : Math.abs(encode().value(user.id)).toString();

      let sambaNTPassword =
        db[upName] &&
        db[upName].hasOwnProperty("sambaNTPassword") &&
        user.accountEnabled
          ? db[upName].sambaNTPassword
          : "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
      let sambaPwdLastSet =
        db[upName] &&
        db[upName].hasOwnProperty("sambaPwdLastSet") &&
        user.accountEnabled
          ? db[upName].sambaPwdLastSet
          : 0;

      if (
        typeof db["tmp_user_to_groups"][user.id] === "undefined" ||
      ...
      for (
        let j = 0, jlen = db["tmp_user_to_groups"][user.id].length;
        j < jlen;
        j++
      ) {
        let g = db["tmp_user_to_groups"][user.id][j];
        if (!db[g].member.includes(upName)) db[g].member.push(upName);
      ...
      let userDisabled =
        user.hasOwnProperty("accountEnabled") && user.accountEnabled === false;
      ...
      if (
        user.hasOwnProperty("customSecurityAttributes") &&
        user.customSecurityAttributes
      ) {
        let flattendAttributes = helper.flattenObjectAndIgnoreOdata({
          cusSecAtt: user.customSecurityAttributes,
        });
...
async function mergeAzureDeviceEntries(db) {
  if (config.LDAP_GETDEVICES) {
    helper.log(
      "database.js",
      "mergeAzureDevicesEntries",
      "try fetching the devices"
    );
    ...
    for (let i = 0, len = devices.length; i < len; i++) {
      ...
      if (deviceDisplayName !== deviceDisplayNameClean) {
        helper.warn(
          "database.js",
          "device names may not contain any special chars. We are using ",
          deviceDisplayNameClean,
          "instead of",
          deviceDisplayName
        );
      }
      ...
      let devName =
        "cn=" + deviceDisplayNameClean + "," + config.LDAP_DEVICESDN;
      devName = devName.toLowerCase();

      renameEntryByUUID(db, device.id, devName);

      if (
        typeof db["tmp_device_to_groups"][device.id] === "undefined" ||
      ...
      for (
        let j = 0, jlen = db["tmp_device_to_groups"][device.id].length;
        j < jlen;
        j++
      ) {
        let g = db["tmp_device_to_groups"][device.id][j];
        let gp = Object.values(db).find(
          (x) => x.entryDN.includes(g) && x.objectClass.includes("posixGroup")
        ).entryDN;
        //helper.log("database.js", "gp", gp);
        if (gp) {
          //helper.log("database.js", "instance of gp", gp);
          if (!db[gp].member.includes(devName)) db[gp].member.push(devName);
        }
      }

      db[devName] = {
        // default values
        objectClass: ["top", "device", "extensibleObject"],
...
database.init = async function (callback) {
  ...
  const interval_func = async function () {
    helper.forceLog(
      "database.js",
      "every",
      config.LDAP_SYNC_TIME,
      "minutes refreshDBentries()"
    );
    await refreshDBentries();
    callback();
  };
  ...
  if (refreshInterval > 0) return setInterval(interval_func, refreshInterval);
  else return null;
};
 ...
 */
database.getSchemaEntries = function () {
